...
{ 
    ...
    public function selectUser($userId, $name)
    {
        ...
        $this->duedate_id = null; // Reset initially
        $this->user_id = $userId;
        $this->users = [];
     
        // Fetch user's apartment
        $apartment = Appartment::where('renter_id', $userId)->first();
        
        // Fetch due dates (both 'not paid' and 'Rejected'), ordered by due date (ascending)
        $this->dueDates = DueDate::where('user_id', $userId)
            ->whereIn('status', ['not paid', 'Rejected'])
        ...
        // Automatically select the earliest due date if available
        if ($this->dueDates->isNotEmpty()) {
            $this->duedate_id = $this->dueDates->first()->id;
        } else {
