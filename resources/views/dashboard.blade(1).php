...
<x-app-layout>
  <div class="py-6">
    <div class="min-w-full mx-auto">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="flex flex-col">
                          ...
                          <tbody class="bg-white divide-y divide-gray-200">
                              @foreach($expiringLease as $lease)
                                  <tr class="hover:bg-gray-50">
                                      ...
                                      <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">
                                            {{ \Carbon\Carbon::parse($lease->check_in)->addMonths($lease->rental_period)->format('M d, Y') }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                            {{ now()->diffInDays(\Carbon\Carbon::parse($lease->check_in)->addMonths($lease->rental_period)) <= 7 ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800' }}">
                                            {{ now()->diffInDays(\Carbon\Carbon::parse($lease->check_in)->addMonths($lease->rental_period)) }} days
                                        </span>
                                    </td>
                                  </tr>
                              @endforeach
          ...
          {{-- <div class="bg-white p-6 pb-6 rounded-lg shadow-xl">
            <h2 class="text-xl font-bold">Building Occupancy</h2>
            <div class="w-full h-64">
    ...
    // Building Occupancy Chart (if you uncomment it)
    const buildingCtx = document.getElementById('buildingOccupancyChart');
    if (buildingCtx) {
